...
const PRESETS: Record<SoundName, (e: SoundEngine, when: number, gain: number) => void> = {
  hover: (e, when, gain) => {
    e.tone(
      { freq: 2100, type: "sine", dur: 0.09, attack: 0.004, release: 0.08, peak: 0.025, detune: (Math.random() - 0.5) * 20, filter: { type: "highpass", freq: 900 }, reverb: 0.18, pan: 0.15 },
      when,
      gain,
    );
  },
  tap: (e, when, gain) => {
    e.tone({ freq: 660, type: "sine", dur: 0.1, attack: 0.004, release: 0.09, peak: 0.05, reverb: 0.16 }, when, gain);
    e.noise({ dur: 0.05, type: "highpass", freq: 3200, peak: 0.03, reverb: 0.1 }, when, gain);
  },
  open: (e, when, gain) => {
    e.tone({ freq: 523.25, type: "triangle", dur: 0.24, attack: 0.008, release: 0.24, peak: 0.06, reverb: 0.42, pan: -0.2 }, when, gain);
    e.tone({ freq: 783.99, type: "sine", dur: 0.3, attack: 0.01, release: 0.3, peak: 0.05, reverb: 0.5, pan: 0.2 }, when + 0.06, gain);
  },
  close: (e, when, gain) => {
    e.tone({ freq: 659.25, type: "triangle", dur: 0.22, attack: 0.008, release: 0.22, peak: 0.055, reverb: 0.38, pan: 0.2 }, when, gain);
    e.tone({ freq: 440, type: "sine", dur: 0.28, attack: 0.01, release: 0.28, peak: 0.05, reverb: 0.44, pan: -0.2 }, when + 0.06, gain);
  },
  confirm: (e, when, gain) => {
    e.tone({ freq: 587.33, type: "sine", dur: 0.34, attack: 0.01, release: 0.34, peak: 0.06, reverb: 0.5, pan: -0.12 }, when, gain);
    e.tone({ freq: 880, type: "sine", dur: 0.36, attack: 0.012, release: 0.36, peak: 0.05, reverb: 0.55, pan: 0.12 }, when, gain);
  },
  toggle: (e, when, gain) => {
    e.tone({ freq: 880, type: "sine", dur: 0.12, attack: 0.005, release: 0.11, peak: 0.05, reverb: 0.22 }, when, gain);
  },
  next: (e, when, gain) => {
    e.tone({ freq: 988, type: "sine", dur: 0.1, attack: 0.004, release: 0.09, peak: 0.045, glideTo: 1180, reverb: 0.2, pan: 0.1 }, when, gain);
  },
  error: (e, when, gain) => {
    e.tone({ freq: 174.61, type: "sine", dur: 0.4, attack: 0.012, release: 0.4, peak: 0.07, reverb: 0.4 }, when, gain);
    e.tone({ freq: 130.81, type: "triangle", dur: 0.44, attack: 0.014, release: 0.44, peak: 0.05, reverb: 0.36 }, when + 0.02, gain);
  },
};
